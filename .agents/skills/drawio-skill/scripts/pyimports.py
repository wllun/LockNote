...
def edges_of(name, path, modules):
    """Map each intra-project module imported by `name` to the first line of
    the import statement that pulls it in."""
    pkg = (
        name
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):  # import a.b.c
            for alias in node.names:
                ...
                if target:
                    found.setdefault(target, node.lineno)
...
def main():
    ap = argparse.ArgumentParser(
        description="Python import graph -> autolayout graph JSON."
    )
    ...
    edge_prov = {
        (name, t): {"path": os.path.relpath(path, args.project), "line": line}
        for name, path in modules.items()
        for t, line in edges_of(name, path, modules).items()
    }
    edges = sorted(edge_prov)
    raw = len(edges)
    ...
    def node(m):
        d = {
            "id": m,
            "label": label(m),
            "provenance": {"path": os.path.relpath(modules[m], args.project)},
        }
        if args.group:
            rest = label(m).split(".")
            if len(rest) > 1:  # nested under a sub-package
                d["group"] = "/".join(
                    rest[:-1]
                )  # full sub-package path -> nested boxes
        return d
    ...
    graph = {
        ...
        "edges": [
            {"source": s, "target": t, "provenance": edge_prov[(s, t)]}
            for s, t in edges
        ],
    }
