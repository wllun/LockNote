...
def discover(root, modpath):
    ...
    for dirpath, dirs, files in os.walk(root):
        dirs[:] = [
            d for d in dirs if d not in ("vendor", "testdata") and not d.startswith(".")
        ]
        gofiles = [
...
def main():
    ap = argparse.ArgumentParser(
        description="Go import graph -> autolayout graph JSON."
    )
    ...
    edges = sorted(
        {
            (ip, t)
            for ip, files in pkgs.items()
            for t in imports_of(files, modpath, pkgs)
            if t != ip
        }
    )
    raw = len(edges)
    ...
    strip = modpath + "/"
    label = (
        lambda ip: ip[len(strip) :] if ip.startswith(strip) else os.path.basename(ip)
    )
    ...
    def node(ip):
        d = {
            ...
            "label": label(ip),
            "provenance": {
                "path": os.path.relpath(os.path.dirname(pkgs[ip][0]), args.module)
            },
        }
        if args.group:
            rest = label(ip).split("/")
