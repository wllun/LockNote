...
create index if not exists idx_private_folders_owner_parent
  on public.private_folders(owner_id, parent_id)
...
begin
  ...
  insert into public.private_folders as current (
    owner_id, local_id, parent_id, name, password_hash, is_pinned, is_archived,
    is_deleted, client_created_at, client_updated_at, server_updated_at
  )
  select
    v_owner_id, item.id, item.parent_id, coalesce(item.name, ''), item.password,
    coalesce(item.is_pinned, false), coalesce(item.is_archived, false),
    coalesce(item.is_deleted, false), coalesce(item.created_at, item.updated_at),
    item.updated_at, now()
  from jsonb_to_recordset(coalesce(p_folders, '[]'::jsonb)) as item(
    id text, parent_id text, name text, password text, is_pinned boolean,
    is_archived boolean, is_deleted boolean, created_at timestamptz,
    updated_at timestamptz
  )
  where item.id is not null and item.updated_at is not null
  on conflict (owner_id, local_id) do update set
    parent_id = case when excluded.is_deleted then current.parent_id else excluded.parent_id end,
  ...
  with recursive removed_folders as (
    select local_id, client_updated_at
    from public.private_folders
    where owner_id = v_owner_id and is_deleted = true
    union
    select child.local_id, greatest(child.client_updated_at, parent.client_updated_at)
    from public.private_folders child
    join removed_folders parent on child.parent_id = parent.local_id
    where child.owner_id = v_owner_id
  )
  update public.private_folders as folder
  set is_deleted = true, is_archived = false,
      client_updated_at = greatest(folder.client_updated_at, removed.client_updated_at),
      server_updated_at = now()
  from removed_folders removed
  where folder.owner_id = v_owner_id and folder.local_id = removed.local_id;
  ...
  where folder.owner_id = v_owner_id and folder.is_deleted = true
    and note.owner_id = folder.owner_id and note.folder_id = folder.local_id
    and note.is_deleted = false;

  select jsonb_build_object(
    'folders', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', folder.local_id, 'parent_id', folder.parent_id, 'name', folder.name,
        'password', folder.password_hash, 'is_pinned', folder.is_pinned,
        'is_archived', folder.is_archived, 'is_deleted', folder.is_deleted,
        'created_at', folder.client_created_at, 'updated_at', folder.client_updated_at
      ) order by folder.local_id)
      from public.private_folders folder where folder.owner_id = v_owner_id
    ), '[]'::jsonb),
    'notes', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', note.local_id, 'folder_id', note.folder_id, 'title', note.title,
        'content', note.content, 'note_type', note.note_type,
        'password', note.password_hash, 'is_pinned', note.is_pinned,
        'is_archived', note.is_archived, 'is_deleted', note.is_deleted,
        'collaboration', note.collaboration, 'created_at', note.client_created_at,
        'updated_at', note.client_updated_at
      ) order by note.local_id)
      from public.private_notes note where note.owner_id = v_owner_id
    ), '[]'::jsonb)
  ) into v_result;
  return v_result;
end;
