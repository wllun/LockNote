-- Store image presentation separately from the optimized source dimensions.
-- Height is always derived from the source aspect ratio in the client.

alter table public.note_attachments
  add column if not exists display_width_ratio real not null default 1;

alter table public.note_attachments
  drop constraint if exists note_attachments_display_width_ratio_check;
alter table public.note_attachments
  add constraint note_attachments_display_width_ratio_check
  check (display_width_ratio between 0.35 and 1);

drop function if exists public.register_note_attachment(
  text, text, uuid, text, text, integer, integer, integer, integer, integer
);
...
create or replace function public.register_note_attachment(
  ...
  p_display_order integer, p_anchor_offset integer, p_display_width_ratio real
...
begin
  ...
  if p_mime_type <> 'image/jpeg' or p_byte_size < 0 or p_byte_size >= 1048576
      ...
      or p_anchor_offset < 0 or p_display_width_ratio not between 0.35 and 1 then
  ...
  insert into public.note_attachments (
    ...
    byte_size, width, height, display_order, anchor_offset, display_width_ratio,
    created_by, updated_at
  ) values (
    p_id, v_owner_id, p_local_note_id, p_shared_note_id, p_storage_path, p_mime_type,
    p_byte_size, p_width, p_height, p_display_order, p_anchor_offset, p_display_width_ratio,
    auth.uid(), now()
    ...
    display_order = excluded.display_order, anchor_offset = excluded.anchor_offset,
    display_width_ratio = excluded.display_width_ratio, updated_at = now()
...
create or replace function public.list_note_attachments(
  ...
  display_width_ratio real, created_at timestamptz, updated_at timestamptz
) language sql stable security definer set search_path = public
as $$
  select a.id, a.storage_path, a.mime_type, a.byte_size, a.width, a.height,
    a.display_order, a.anchor_offset, a.display_width_ratio, a.created_at, a.updated_at
...
as $$ begin
  if coalesce(jsonb_array_length(p_layout), 0) > 20 or auth.uid() is null
    ...
    raise exception 'You cannot update images in this note' using errcode = '42501';
  end if;
  update public.note_attachments a set
    display_order = positions.display_order,
    anchor_offset = positions.anchor_offset,
    display_width_ratio = positions.display_width_ratio,
    updated_at = now()
  from jsonb_to_recordset(coalesce(p_layout, '[]'::jsonb))
    as positions(id text, display_order integer, anchor_offset integer, display_width_ratio real)
  where positions.display_order between 0 and 19 and positions.anchor_offset >= 0
    and positions.display_width_ratio between 0.35 and 1
...
revoke all on function public.register_note_attachment(text,text,uuid,text,text,integer,integer,integer,integer,integer,real) from public, anon;
...
grant execute on function public.register_note_attachment(text,text,uuid,text,text,integer,integer,integer,integer,integer,real) to authenticated;
