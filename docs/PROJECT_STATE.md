...
### Possible features
- [X] Premium purchase module — the third bottom tab loads RevenueCat's Current offering, displays localized store prices, requires a Free LockNote account, completes Plus/Pro checkout, restores purchases, opens provider subscription management, reacts to customer-info/foreground updates, and derives the current plan from verified entitlements. `react-native-purchases` requires a native development build for real mobile payments; Expo Go can use RevenueCat Test Store. External store/RevenueCat products and public SDK keys still require setup. Premium feature gating, quotas, and server-owned entitlement persistence remain unimplemented. See [Subscription Plans](decisions/SUBSCRIPTION_PLANS.md) and [Subscription Payment Setup](decisions/SUBSCRIPTION_SETUP.md).
