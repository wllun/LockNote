...
- [X] Collaboration Release 1 — explicit per-note sharing by registered account email, per-recipient Can edit/View only access, owner share indicators, collaborator management, realtime refresh, last-editor footer, RLS, revision-protected saves, and one-editor-at-a-time server leases. Editors renew a 90-second lease every 30 seconds; other accounts remain view-only, pending drafts are preserved before network saves, and abandoned leases expire automatically. The Shared tab fetches only from Supabase and displays no cached notes while offline. View-only access and active lease ownership are enforced in every editor and by the Supabase save RPC. The new lease migration and live two-account verification still require deployment/configured Supabase credentials. The planned policy requires Plus or Pro for the owner while invited collaborators only need a Free account; this entitlement is not enforced yet.
...
### Possible features
- [X] Premium purchase module — the third bottom tab loads RevenueCat's Current offering, displays localized store prices, requires a Free LockNote account, completes Plus/Pro checkout, restores purchases, opens provider subscription management, opens the Privacy Policy and Terms of Service as offline in-app pages, reacts to customer-info/foreground updates, and derives the current plan from verified entitlements. Matching public HTML copies are ready under `docs/legal/` for free GitHub Pages hosting. `react-native-purchases` requires a native development build for real mobile payments; Expo Go can use RevenueCat Test Store. External store/RevenueCat products, public SDK keys, publishing the legal pages, final operator/support details, and legal review still require setup. Premium feature gating, quotas, and server-owned entitlement persistence remain unimplemented. See [Subscription Plans](decisions/SUBSCRIPTION_PLANS.md) and [Subscription Payment Setup](decisions/SUBSCRIPTION_SETUP.md).
...
- [X] Sync DB — Profile screen pushes and pulls private/owned notes and folders through an account-scoped Supabase RPC. Deletions and root-note semantics are preserved. Manual sync is planned for every account with plan limits of 25 MB Free, 75 MB Plus, and 750 MB Pro including images; enforcement is not implemented.
