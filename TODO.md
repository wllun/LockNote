...
Use the [documentation index](docs/README.md), [current project state](docs/PROJECT_STATE.md)
and [test plan](docs/testing/TEST_PLAN.md). Implemented features are not evidence
that production secrets, migrations, store products or physical-device tests are
complete. Markdown is current guidance; older Word/PDF companions may lag.
...
- [ ] Rebuild Android/iOS binaries and complete the [automatic/background acceptance matrix](docs/testing/TEST_PLAN.md#5-automaticbackground-sync-acceptance-and-future-exclusions), including editor deferral, offline retries, identity guards, quotas/expiry and observed OS tasks. Automatic record sync is implemented, not guaranteed on an exact schedule.
- [ ] Decide whether to restore the hidden Settings Export Backup action. The service is implemented and Import Backup is visible; do not claim an export button exists or uninstall the only data copy based on that assumption.
- [ ] Complete the in-app account-deletion flow, associated cloud cleanup and public request information before a store release that supports account creation.
- [ ] Update Word/PDF testing/learning/proposal companions separately if they are still maintained deliverables; do not label them regenerated by a Markdown-only refresh.
