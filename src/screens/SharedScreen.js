...
const SharedScreen = ({ navigation }) => {
  ...
  if (!isSupabaseConfigured || !session) return <View style={styles.center}><View style={styles.emptyIcon}><Ionicons name="people-outline" size={34} color={colors.primary} /></View><Text style={styles.emptyTitle}>Shared with me</Text><Text style={styles.emptyText}>{!isSupabaseConfigured ? 'Account services need to be configured for sharing.' : 'Sign in from Profile to see notes others share with you.'}</Text></View>;
