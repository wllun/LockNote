...
export const formatExpenseCategoryPeriod = (month, year) => {
  const numericMonth = Number(month);
  const numericYear = Number(year);
  if (!/^\d{1,2}$/.test(String(month)) || numericMonth < 1 || numericMonth > 12
    || !/^\d{4}$/.test(String(year)) || numericYear < 1000) return null;
...
export const buildNoteExportHtml = ({
  ...
  const categoryRows = getExpenseExportRows(categoryExport?.rows);
  ...
  const exportAttachments = (Array.isArray(attachments) ? attachments : [])
    .filter((item) => item?.data_uri || item?.local_uri)
    .slice(0, 20);
  const inlineNoteHtml = groupInlineNoteBlocks(buildInlineNoteBlocks(content, exportAttachments))
    .map((block) => block.type === 'text'
      ? (block.text ? `<div class="content">${preserveLineBreaks(escapeHtml(block.text))}</div>` : '')
      : `<div class="inline-attachment-row">${block.blocks.map((imageBlock) => `<img class="inline-attachment" style="width:calc(${imageBlock.attachment.display_width_ratio * 100}% - 6px)" src="${escapeHtml(imageBlock.attachment.data_uri || imageBlock.attachment.local_uri)}" alt="Note attachment">`).join('')}</div>`)
    .join('') || '<div class="content">This note is empty.</div>';
  const body = isExpense
    ...
    : isCategoryExport
      ? `<section class="daily-expenses first-section"><h2>${escapeHtml(period || 'Period not selected')}</h2>${recordTitle ? `<p style="margin:0 0 16px;color:#687086">Expense record: ${escapeHtml(recordTitle)}</p>` : ''}<table><thead><tr><th>Day</th><th>Remark</th><th class="amount">${escapeHtml(currencySymbol)}</th></tr></thead><tbody>${categoryRows.length
        ? categoryRows.map((row) => `<tr><td>${escapeHtml(row.date || '-')}</td><td>${escapeHtml(row.remark || 'No remark')}</td><td class="amount">${escapeHtml(row.amount || '0.00')}</td></tr>`).join('')
        : '<tr><td colspan="3">No matching transactions</td></tr>'}</tbody></table></section>`
    : isChecklist
