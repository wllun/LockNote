...
import { formatCollaborativeEdit, isReadOnlyCollaborativeNote } from '../utils/collaboration-note.mjs';
...
const CollaborationFooter = ({ noteId, onRemoteNote }) => {
  ...
  const isReadOnly = isReadOnlyCollaborativeNote(note);
  ...
  return <View style={styles.container}>
    ...
    {note.sync_status === 'conflict' && <View style={styles.actions}><Pressable disabled={resolving} onPress={() => resolve('remote')}><Text style={styles.actionText}>Use latest</Text></Pressable>{!isReadOnly && <Pressable disabled={resolving} onPress={() => resolve('local')}><Text style={styles.actionText}>Keep mine</Text></Pressable>}</View>}
  </View>;
};
